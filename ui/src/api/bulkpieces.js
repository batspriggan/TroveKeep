...
export const getAllBulkPieces = (page = 1, size = 50, q = '', assigned = null) => {
  let url = `${BASE}?page=${page}&size=${size}&q=${encodeURIComponent(q)}`
  if (assigned === true || assigned === false) url += `&assigned=${assigned}`
  return get(url)
}
